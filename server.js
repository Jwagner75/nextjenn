...
app.post('/api/create-interview', async (req, res) => {
  try {
    const { company_name, hiring_manager_name, job_title, cand_name, cand_email, client_email } = req.body;
    if (!job_title || !cand_name || !cand_email || !client_email) {
      return res.status(400).json({ error: 'All fields required' });
    }

    const session_id = cand_name.toLowerCase().replace(/\s+/g,'-') + '-' + Date.now();
    const params = new URLSearchParams({ session: session_id, job: job_title, name: cand_name, client: client_email, company: company_name || '', manager: hiring_manager_name || '' });
...
// ── FIRE TRANSCRIPT EMAIL
async function fireTranscriptEmail(session_id) {
  ...
  console.log('Sending transcript email to ' + toEmail + ' for ' + cand_name);
  ...
  const html = '<!DOCTYPE html><html><head><meta charset="UTF-8"></head>'
    ...
    + '<table width="100%" cellpadding="0" cellspacing="0" style="padding:32px 16px;"><tr><td align="center">'
    + '<table width="600" cellpadding="0" cellspacing="0" style="background:#fff;border-radius:16px;overflow:hidden;max-width:600px;">'
    + '<tr><td style="background:#1B2A4A;padding:24px 32px;"><span style="font-size:20px;font-weight:700;color:#fff;letter-spacing:0.08em;">NEXT-JENN</span>'
    + '<span style="float:right;font-size:11px;color:#8892A4;letter-spacing:0.1em;text-transform:uppercase;">AI Recruiter Interview</span></td></tr>'
    ...
    + '<td align="right"><p style="margin:0;font-size:18px;font-weight:700;color:#1B2A4A;">' + (job_title||'Open Role') + '</p>'
    + (company_name ? '<p style="margin:2px 0 0;font-size:13px;color:#555B6E;">' + company_name + '</p>' : '')
    + '<p style="margin:2px 0 0;font-size:13px;color:#555B6E;">' + dateStr + '</p></td></tr></table></td></tr>'
    + '<tr><td style="padding:24px 32px;"><p style="margin:0 0 16px;font-size:11px;font-weight:700;color:#8892A4;text-transform:uppercase;letter-spacing:0.15em;">Interview Responses</p>'
    + qaHtml
    + '<p style="margin:16px 0 0;font-size:12px;color:#8892A4;padding:12px;background:#F8F9FC;border-radius:8px;border:1px solid #D8DCE6;">Click Watch Response to view each video clip. Interview conducted by Jenn, AI Recruiter - Next-Jenn Platform.</p>'
