...
var LABELS = {
  q1:    'Q2 — What is your interest in this position?',
  q2:    'Q3 — 3 qualifications that make you the best fit',
  q3:    'Q4 — Project you are most proud of',
  q4:    'Q5 — Summary of experience as it pertains to this role',
  final: 'Q6 — Are you interested in moving forward?',
};
...
app.post('/api/confirm-schedule', async function(req, res) {
  try {
    ...
    // Use timezone from request body, fallback to detecting from offset
    var tz = req.body.timezone || 'America/Chicago';
    var timeStr;
    try {
      timeStr = sendAt.toLocaleString('en-US', {
        timeZone: tz,
...
app.post('/api/upload', upload.single('video'), async function(req, res) {
  try {
    ...
    console.log('Session ' + sessionId + ': ' + count + '/5 videos — hasAll:' + hasAll);
