...
function sendTranscript(sessionId) {
  ...
  // Send transcript to hiring manager
  mailer.sendMail({
    from:    fromAddr(),
    to:      toEmail,
  ...
  var candEmail = session.meta.cand_email || '';
  if (candEmail) {
    var candHtml = emailWrap(
      ...
      + '</div>'
      + '<p style="margin:0 0 16px;font-size:14px;color:#555B6E;line-height:1.7;">'
      + 'The hiring manager will review your video responses and reach out if they would like to move forward in the process.'
      + '</p>'
      + '<p style="margin:0;font-size:13px;color:#8892A4;font-style:italic;">— Jenn, AI Recruiter · Next-Jenn</p>'
    );
    mailer.sendMail({
      from:    fromAddr(),
      to:      candEmail,
      subject: 'Your Interview Has Been Submitted — ' + job + ' | Next-Jenn',
      html:    candHtml,
    }, function(err) {
      if (err) console.error('Candidate confirmation error: ' + err.message);
      else console.log('Candidate confirmation sent to ' + candEmail);
    });
  }
}
